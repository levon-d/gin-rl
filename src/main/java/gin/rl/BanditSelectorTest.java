...
/**
 * Simple test harness for bandit selector implementations.
 *
 * This class simulates the RL loop without requiring the full GIN framework,
 * allowing us to verify that the bandit algorithms behave correctly.
 *
 * Run with: java -cp build/classes/java/main gin.rl.BanditSelectorTest
 * Or after building jar: java -cp gin.jar gin.rl.BanditSelectorTest
 */
public class BanditSelectorTest {

    // Simulated operator "true" qualities (unknown to the algorithms)
    // Higher = better (simulates that some operators are more effective)
    private static final Map<Class<? extends Edit>, Double> TRUE_QUALITIES = new HashMap<>();

    static {
        // Traditional operators - varying effectiveness
        TRUE_QUALITIES.put(DeleteStatement.class, 0.3);      // Rarely helpful
        TRUE_QUALITIES.put(CopyStatement.class, 0.5);        // Sometimes helpful
        TRUE_QUALITIES.put(ReplaceStatement.class, 0.7);     // Often helpful
        TRUE_QUALITIES.put(SwapStatement.class, 0.4);        // Occasionally helpful
        TRUE_QUALITIES.put(MoveStatement.class, 0.35);       // Rarely helpful
        TRUE_QUALITIES.put(BinaryOperatorReplacement.class, 0.6);  // Moderately helpful
        TRUE_QUALITIES.put(UnaryOperatorReplacement.class, 0.45);  // Occasionally helpful

        // Matched operators
        TRUE_QUALITIES.put(MatchedDeleteStatement.class, 0.35);
        TRUE_QUALITIES.put(MatchedCopyStatement.class, 0.55);
        TRUE_QUALITIES.put(MatchedReplaceStatement.class, 0.75);  // Best traditional
    ...
    /**
     * Simulate applying an operator. Reward is drawn from a distribution
     * centered around the operator's "true quality" with noise.
     */
    ...
    /**
     * Run a single selector for a number of steps and return statistics.
     */
